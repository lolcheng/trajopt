...
用法: python3 plot_seg_trajectory.py [输出目录] [--rbf rbf.json路径] [--margin 0.8] [--initial-only] [--no-show]
...
若缺少 terrain_grid.txt，则会尝试从 rbf.json 计算轨迹附近的高程图。
...
def load_rbf_params(path):
    """读取 rbf.json，返回 (centers_x, centers_y, weights, sigma) 或 None。"""
    if not path or not os.path.isfile(path):
        return None
    with open(path, 'r', encoding='utf-8') as f:
        data = json.load(f)
    grid = data.get('cur_rbf_grid', [])
    weights = data.get('rbf_weight', [])
    if not grid or not weights or len(grid) != len(weights):
        return None
    centers_x = np.array([p[0] for p in grid], dtype=float)
    centers_y = np.array([p[1] for p in grid], dtype=float)
    weights = np.array(weights, dtype=float)
    sigma = float(data.get('sigma', 0.14))
    return centers_x, centers_y, weights, sigma


def eval_rbf_height(X, Y, centers_x, centers_y, weights, sigma):
    """向量化计算 RBF 高程。"""
    Z = np.zeros_like(X, dtype=float)
    denom = 2.0 * sigma * sigma
    for cx, cy, ww in zip(centers_x, centers_y, weights):
        d2 = (X - cx) ** 2 + (Y - cy) ** 2
        Z += ww * np.exp(-d2 / denom)
    return Z


def _traj_xy_bounds(init, final):
    xs, ys = [], []
    for tr in (init, final):
        if tr is None:
            continue
        xs.extend([tr['xb'], tr['xl'], tr['xr']])
        ys.extend([tr['yb'], tr['yl'], tr['yr']])
    if not xs:
        return None
...
def crop_or_build_nearby_terrain(terrain, init, final, margin=0.8, rbf=None):
    """
    优先使用已有 terrain_grid 并裁剪到轨迹附近；
    若不存在 terrain_grid，则基于 rbf.json 在轨迹附近重建高程网格。
    """
    bounds = _traj_xy_bounds(init, final)
    if bounds is None:
        return terrain
    ...
    if terrain is not None:
        ...
        x1 = X[0, :]
        y1 = Y[:, 0]
        xi = np.where((x1 >= xmin) & (x1 <= xmax))[0]
        yi = np.where((y1 >= ymin) & (y1 <= ymax))[0]
        if xi.size >= 2 and yi.size >= 2:
            return X[np.ix_(yi, xi)], Y[np.ix_(yi, xi)], Z[np.ix_(yi, xi)]
        return terrain

    if rbf is None:
        return None
    centers_x, centers_y, weights, sigma = rbf
    nx, ny = 120, 120
    x = np.linspace(xmin, xmax, nx)
    y = np.linspace(ymin, ymax, ny)
    X, Y = np.meshgrid(x, y)
    Z = eval_rbf_height(X, Y, centers_x, centers_y, weights, sigma)
    return X, Y, Z
...
def main():
    ...
    rbf_file = None
    margin = 0.8
    initial_only = '--initial-only' in args
    i = 0
    while i < len(args):
        if args[i] == '--initial-only':
            i += 1
            continue
        if args[i] == '--rbf' and i + 1 < len(args):
            rbf_file = args[i + 1]
            i += 2
            continue
    ...
    if rbf_file is None:
        candidates = [
            os.path.join(out_dir, 'rbf.json'),
            os.path.join(os.getcwd(), 'test', 'terrain_res', 'rbf.json'),
            os.path.join(os.getcwd(), '..', 'test', 'terrain_res', 'rbf.json'),
        ]
        for c in candidates:
            if os.path.isfile(c):
                rbf_file = c
                break
    rbf_params = load_rbf_params(rbf_file) if rbf_file else None
    terrain = crop_or_build_nearby_terrain(terrain, init, final, margin=margin, rbf=rbf_params)
