...
void SegTrajOptNLP::extractTrajectory(const Ipopt::Number* x_in,
    ...
    const double* clx = x + idx_clx_, * cly = x + idx_cly_;
    const double* crx = x + idx_crx_, * cry = x + idx_cry_;
    ...
    for (int i = 0; i < n_sample_; ++i) {
        if (terrain_) {
            Zl[i] = terrain_->height(Xl[i], Yl[i]);
            Zr[i] = terrain_->height(Xr[i], Yr[i]);
        } else {
            Zl[i] = 0.0;
            Zr[i] = 0.0;
        }
...
void SegTrajOptNLP::extractCoefficients(const Ipopt::Number* x_in,
    ...
    clx.resize(n_coeff_); cly.resize(n_coeff_); clz.resize(n_coeff_);
    crx.resize(n_coeff_); cry.resize(n_coeff_); crz.resize(n_coeff_);
    ...
    std::fill(clz.begin(), clz.end(), 0.0);
    std::fill(crz.begin(), crz.end(), 0.0);
}
